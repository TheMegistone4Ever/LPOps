...
def extract_plot_data(data: List[Tuple], model_func: callable, params: ndarray) -> Dict[str, Any]:
    ...
    for n in plot_data["n_vals"]:
        ...
        if params is not None:
            m_range = linspace(min(plot_data["m_vals"]), max(plot_data["m_vals"]), 100)
            plot_data["model_fits_m"][n] = (m_range, model_func((m_range, full_like(m_range, n)), *params))
    ...
    for m in plot_data["m_vals"]:
        ...
        if params is not None:
            n_range = linspace(min(plot_data["n_vals"]), max(plot_data["n_vals"]), 100)
            plot_data["model_fits_n"][m] = (n_range, model_func((full_like(n_range, m), n_range), *params))

    # 3D Plots Data
    if params is not None:
        df_grouped = df.groupby(["m", "n"])["operations"].mean().reset_index()
        plot_data["m_grid"], plot_data["n_grid"] = meshgrid(plot_data["m_vals"], plot_data["n_vals"])
...
def create_plots(plot_data: Dict[str, Any], model_name: str, params: ndarray, plot_dir: str = "plots",
                 sample_id: str = ""):
    """Create visualization plots with box plots and connected average lines."""

    makedirs(plot_dir, exist_ok=True)

    safe_name = sub(r"[\\/*?:'<>|]", "", model_name)
